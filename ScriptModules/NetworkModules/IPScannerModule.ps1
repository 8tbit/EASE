...
using module '.\NetworkDetectionModule.psm1'

[CmdletBinding(DefaultParameterSetName='CIDR')]
Param(
    [Parameter(
        ParameterSetName='AutoDetectIPData',
        Position=0,
        Mandatory=$false,
        HelpMessage='Auto Detct Network Settings'
    )]
    [switch]$AutoDetectNetworkSettings,
...
Begin{
    ...
    if($AutoDetectNetworkSettings)
    {
        ...
        [NetworkInterface.NetworkAdapters]$OAdapters = [NetworkInterface.NetworkAdapters]::new()
        $OAdapters = Get-NetworkSettings

        foreach($adapterinterface in $OAdapters.NetworkAdapters)
        {
            $NSet = [NetworkInterface.NetworkSettings]::new()
            $NSet = [NetworkInterface.NetworkSettings]$adapterinterface

            if($NSet.DefaultGateway -eq "")
            {
            continue

            $RetryCount += 1
            $NSet = [NetworkInterface.NetworkSettings]$OAdapters.NetworkAdapters[$RetryCount]
            }


            if($EnableMACResolving)
            {
                #WARNING, LONG TIMES MAY ACRUE FOR HIGH NETWORK VOLUMES
                $results = .\IPScannerModule.ps1 -IPv4Address $NSet.DefaultGateway -Mask "255.255.255.0" -EnableMACResolving -Tries $Tries -Threads $Threads
            }

            else
            {
                $results = .\IPScannerModule.ps1 -IPv4Address $NSet.DefaultGateway -Mask $NSet.SubnetMask -Tries $Tries -Threads $Threads
            }
        
            Write-Verbose "`n`n################################"
            Write-Verbose $NSet.InterfaceDescription
...
Process{
   ...
   #Write-Verbose -Message "Setting up RunspacePool..."
    ...
    # Close the RunspacePool and free resourcesip
    $RunspacePool.Close()
